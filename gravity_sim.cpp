    ...
    const double G_SCALED = G * 1e18;
    const float c = 299792458.0;
    float initMass = float(pow(10, 22));
    float sizeRatio = 30000.0f;

	constexpr float AU = 149.6e9f; // meters
    constexpr float ORBITAL_VISUAL_SCALE = 1e-9f;
    constexpr float METERS_PER_UNIT = AU * ORBITAL_VISUAL_SCALE;
    ...
    class Object {
        public:
            ...
            Object(glm::vec3 initPosition, glm::vec3 initVelocity, float mass, float density = 3344, glm::vec4 color = glm::vec4(1.0f, 0.0f, 0.0f, 1.0f), bool Glow = false, bool fixed = false) {
                this->position = initPosition;
                this->velocity = initVelocity;
                this->mass = mass;
                this->density = density;

				// uniform radius scaling based on mass and density
                float physicalRadius = pow((3.0f * mass / density) / (4.0f * 3.14159265359f), 1.0f / 3.0f);
                constexpr float VISUAL_RADIUS_SCALE = 1000.0f;
                constexpr float MIN_VISUAL_RADIUS = 2.0f;
                constexpr float MAX_VISUAL_RADIUS = 80.0f;

				//logarithmic scaling to keep radii manageable
                float renderRadius = log10f(physicalRadius + 1.0f) * 6.0f;
                renderRadius = glm::clamp(renderRadius, 2.5f, 70.0f);

                //this->radius = std::clamp((physicalRadius / sizeRatio) * VISUAL_RADIUS_SCALE, MIN_VISUAL_RADIUS, MAX_VISUAL_RADIUS);
            ...
            void UpdatePos(float dt){
                ...
                //radius = (physicalRadius / sizeRatio) * VISUAL_RADIUS_SCALE;
            }
            void UpdateVertices() {
                // generate new vertices with current radius
                std::vector<float> vertices = Draw();
                vertexCount = vertices.size();   // ← REQUIRED
    ...
    float orbitalVelocity(float r_meters){
        return sqrt(G_SCALED * (1.989e30 * 0.1) / r_meters);
    }
    ...
    int main() {
        ...
        constexpr float ORBIT_SCALE = 0.1f;
        constexpr float VELOCITY_SCALE = 3.162277f;
        constexpr float MASS_SCALE = 1e-9f;

        float mercDist = 0.387f * AU * ORBITAL_VISUAL_SCALE;
        float venusDist = 0.723f * AU * ORBITAL_VISUAL_SCALE;
        float earthDist = 1.0f * AU * ORBITAL_VISUAL_SCALE;
        float marsDist = 1.524f * AU * ORBITAL_VISUAL_SCALE;
        float jupDist = 5.203f * AU * ORBITAL_VISUAL_SCALE;
        float satDist = 9.537f * AU * ORBITAL_VISUAL_SCALE;
        float uraDist = 19.191f * AU * ORBITAL_VISUAL_SCALE;
        float nepDist = 30.07f * AU * ORBITAL_VISUAL_SCALE;

		float r_m = earthDist / METERS_PER_UNIT;
        float v = orbitalVelocity(r_m);
        ...
        objs = {
           ...
           //Object(glm::vec3(0, 0, 0), glm::vec3(0, 0, 0), 5.97219*pow(10, 24), 5515, glm::vec4(0.0f, 1.0f, 1.0f, 1.0f)),
            // ---------------- SUN ----------------
            Object(glm::vec3(0, 0, -35), glm::vec3(0, 0, 0),1.989e30*MASS_SCALE , 5515, glm::vec4(1.0f, 0.929f, 0.176f, 1.0f), true, true),

            // ---------------- MERCURY ----------------
            Object(glm::vec3(mercDist, 0, -35), glm::vec3(0, 1926 * VELOCITY_SCALE, 0),3.301e23 * MASS_SCALE, 5427,glm::vec4(0.7f, 0.7f, 0.7f, 1.0f)),

            // ---------------- VENUS ----------------
            Object(glm::vec3(venusDist, 0, -35), glm::vec3(0, 1574 * VELOCITY_SCALE, 0),4.867e24 * MASS_SCALE, 5240,glm::vec4(1.0f, 0.85f, 0.3f, 1.0f)),

            // ---------------- EARTH ----------------
            Object(glm::vec3(earthDist, 0, -35), glm::vec3(0, v * VELOCITY_SCALE, 0),5.972e24 * MASS_SCALE, 5515,glm::vec4(0.0f, 0.5f, 1.0f, 1.0f)),

            // ---------------- MARS ----------------
            Object(glm::vec3(marsDist, 0, -35), glm::vec3(0, 1192 * VELOCITY_SCALE, 0),6.417e23 * MASS_SCALE, 3930,glm::vec4(1.0f, 0.2f, 0.0f, 1.0f)),

            // ---------------- JUPITER ----------------
            Object(glm::vec3(jupDist, 0, -35), glm::vec3(0, 964 * VELOCITY_SCALE, 0), 1.898e27 * MASS_SCALE, 1326, glm::vec4(0.9f, 0.6f, 0.3f, 1.0f)),

            // ---------------- SATURN ----------------
            Object(glm::vec3(satDist, 0, -35), glm::vec3(0, 891 * VELOCITY_SCALE, 0), 5.683e26 * MASS_SCALE, 687,glm::vec4(0.9f, 0.8f, 0.5f, 1.0f)),

            // ---------------- URANUS ----------------
            Object(glm::vec3(uraDist, 0, -35), glm::vec3(0, 821 * VELOCITY_SCALE, 0), 8.681e25 * MASS_SCALE, 1270, glm::vec4(0.5f, 1.0f, 1.0f, 1.0f)),
                         
            // ---------------- NEPTUNE ----------------
            Object(glm::vec3(nepDist, 0, -35), glm::vec3(0, 767 * VELOCITY_SCALE, 0), 1.024e26 * MASS_SCALE, 1638, glm::vec4(0.3f, 0.3f, 1.0f, 1.0f)),
        ...
        float timeStep = 100.0f;  // adjust to control speed
        float dt = deltaTime * timeStep;
        ...
        while (!glfwWindowShouldClose(window) && running == true) {
            ...
            for(auto& obj : objs) {
                ...
                for(auto& obj2 : objs){
                    if(&obj2 != &obj && !obj.Initalizing && !obj2.Initalizing){
                        ...
                        if (distance > 0) {
                            ...
                            //distance *= 1000;
                            double Gforce = (G_SCALED * obj.mass * obj2.mass) / (distance * distance);
                        

                            float acc1 = Gforce / obj.mass;
                            std::vector<float> acc = {direction[0] * acc1, direction[1]*acc1, direction[2]*acc1};
                            if(!pause && !obj.fixed) {
                                obj.accelerate(acc[0], acc[1], acc[2], timeStep* deltaTime);
                            }
                ...
                if(obj.Initalizing){
                    float physicalRadius = pow((3.0f * obj.mass / obj.density) / (4.0f * 3.14159265359f), 1.0f / 3.0f);
					constexpr float VISUAL_RADIUS_SCALE = 1000.0f;
                    //obj.radius = pow(((3 * obj.mass/obj.density)/(4 * 3.14159265359)), (1.0f/3.0f)) / 1000000;
                    obj.radius = (physicalRadius / sizeRatio) * VISUAL_RADIUS_SCALE;
                ...
                //update positions
                if(!pause && !obj.fixed) {
                    obj.UpdatePos(timeStep * deltaTime);
                }
    ...
    GLFWwindow* StartGLU() {
        ...
        GLFWwindow* window = glfwCreateWindow(1920, 1080, "Gravity Simulation", NULL, NULL);
        if (!window) {
    ...
    std::vector<float> UpdateGridVertices(std::vector<float> vertices, const std::vector<Object>& objs){
        ...
        constexpr float GRID_GRAVITY_SCALE = 1e-4f; // visual tuning original: 0.00000005f
        ...
        for (int i = 0; i < vertices.size(); i += 3) {
            ...
            for (const auto& obj : objs) {
                ...
                glm::vec3 toObject = vertexPos - obj.GetPos();
                float r = glm::length(toObject) + EPS;

                displacementY -= (G_SCALED * obj.mass) / r;
                //float distance = glm::length(toObject);
                //float distance_m = distance * 1000.0f;
                //float rs = (2*G_SCALED*obj.mass)/(c*c);
                //// visual exaggeration only
                //rs *= 1000.0f;
                //float safe = distance_m - rs;
                //if (safe > 0.0f) {
                //    float dz = 2 * sqrt(rs * (distance_m - rs));
                //    totalDisplacement.y += dz * 2.0f;
                //}

                // apply newtonian gravity grid
                /*float r = glm::length(toObject) + EPS;

                float depth = -(G * obj.mass) / r;
                displacement.y += depth * GRID_GRAVITY_SCALE;*/
            }
            vertices[i+1] = displacementY * GRID_GRAVITY_SCALE;
        }
